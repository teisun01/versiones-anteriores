/* --------- PARALLAX --------- */
// Antes del resto de tu código JS:
window.scaleFactor = window.innerWidth / 1920;

// Cuando cambie el tamaño de la pantalla:
...
function update(){
  ...
  if (capitulo1) {
    ...
    const escalaGlobal = window.scaleFactor || 1; // <-- agregado

    // Capas del árbol (SIN cambiar su escala)
    ...
    // descendiente (SIN escalar)
if (descendiente)
  descendiente.style.transform = `translateX(-50%) translateY(${y * 0.15}px)`;


if (frutosInd) {
  const escalaGlobal = window.scaleFactor || 1;
  const imgsFrutos = frutosInd.querySelectorAll('img');
  imgsFrutos.forEach(img => {
    img.style.transform = `translate(-50%, -50%) scale(${escalaExtra * escalaGlobal})`;
...
const frutosRect = frutosInd.getBoundingClientRect();
const descendienteY = frutosRect.bottom - 200 * (window.scaleFactor || 1);

descendiente.style.transform =
  `translateX(-50%) translateY(${descendienteY}px) scale(${escalaExtra * (window.scaleFactor || 1)})`;
}