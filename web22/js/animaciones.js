...
function actualizarEscalaArbolMovil() {
  if (window.innerWidth <= 768) {
    const baseWidth = 768; // ancho base de diseño móvil
    const scale = Math.max(window.innerWidth / baseWidth, 0.7); // mínimo 0.7
    document.documentElement.style.setProperty("--arbol-scale", scale);
  } else {
    // restablece si salís del rango móvil
...
// === 🔧 AJUSTE PROGRESIVO SOLO ENTRE 601–768px ===
(function ajusteVerticalProgresivo() {
  const elementos = [
    { sel: "#layer-descendiente", max: 150 },
    { sel: "#layer-frutos-ind", max: 100 },
    { sel: "#layer-frutos", max: 50 },
    { sel: "#layer-brilloraices", max: 40 },
    { sel: "#contador-wrapper", max: 350 }, // <- IMPORTANT: usar ID
    ...
    if (w >= 601 && w <= 768) {
      ...
      elementos.forEach(({ sel, max }) => {
        ...
        if (sel === "#contador-wrapper") {
          // actualizamos la variable CSS --contador-offset
          el.style.setProperty("--contador-offset", `-${offset}px`);
          // para debug: mostrar valor en consola (comentá si sobra)
          // console.log("contador offset:", `-${offset}px`);
        } else {
          // mover con top (usa la posición base)
          if (!el.dataset.baseTop) {
            const estilo = window.getComputedStyle(el);
            el.dataset.baseTop = estilo.top === "auto" ? "0px" : estilo.top;
          }
      ...
      // restaurar posiciones base fuera del rango
      elementos.forEach(({ sel }) => {
        const el = document.querySelector(sel);
        if (!el) return;

        if (sel === "#contador-wrapper") {
          el.style.removeProperty("--contador-offset"); // vuelve a 0
        } else if (el.dataset.baseTop) {
  ...
  // también ejecutamos una vez ahora para asegurar estado inicial correcto
  aplicarAjuste();
})();
